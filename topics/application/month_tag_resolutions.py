"""
Same file as date_tag_resolutions.py but this time
with month tags included

In a parsed file with primitives, clean the datetag so that
- only events with daily and monthly resolution are considered
...
or
- first date-tag with monthly resolution is considered representative of the event

Uncertainty of how representative a selected date-tag is will
be tracked in the newly created 'date_uncertainty' field

-i "../primitives_230414/primitives_annotated.ndjson"
-o "../primitives_230414/primitives_annotated_monthly.ndjson"
"""
...
def extract_daily_tag(date_tag_list):
    ...
    elif 'month' in resolutions:
        # extract month tag to represent the document
        day_id = resolutions.index('month')
        clean_date_tag = date_tag_list[day_id]

        # date-tag certainty
        # highest degree – only one tag and it's day
        if len(resolutions) == 1 and all(r == 'month' for r in resolutions):
            uncertainty = 'unambiguous month'
        # multiple monthly tags in a single doc
        elif len(resolution) > 1 and all(r == 'month' for r in resolutions):
            uncertainty = 'multiple month events in document'
        # multiple resolutions in a single doc
        elif len(resolutions) > 1 and not all(r == 'month' for r in resolutions):
            uncertainty = 'varying resolutions'
        else:
            uncertainty = 'unknown degree'
